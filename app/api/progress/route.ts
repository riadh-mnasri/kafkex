 ...
 *
 * Auth: the client sends the passphrase in the x-kafkex-passphrase
 * header; it must match the KAFKEX_PASSPHRASE environment variable.
...
function isAuthorized(request: NextRequest): boolean {
  const expected = process.env.KAFKEX_PASSPHRASE;
  if (!expected) return false; // sync disabled until configured
  const given = request.headers.get("x-kafkex-passphrase") ?? "";
