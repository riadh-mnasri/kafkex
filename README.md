...
Keep your Kafka sharp. Kafkex is a bilingual (EN/FR) learning app that
combines expert-level lessons, spaced repetition flashcards and code
exercises covering Apache Kafka, wrapped in a lightweight gamification
loop (XP, levels, daily streak, activity heatmap) designed for short
daily sessions.
...
- **Lessons**: short, dense refreshers with runnable-in-your-head code
  samples, tips and gotchas. Written for people who already know the
  basics of Kafka and want an expert-level refresh.
...
1. Copy `.env.example` to `.env.local` and set `KAFKEX_PASSPHRASE` to a
...
content/packs/kafka/    The Kafka content pack (manifest + modules)
docs/adr/               Architecture decision records
tests/                  Unit tests for the engine
```

Kafkex is forked from Kortex, a topic-agnostic learning app template:
the engine never references Kafka directly, all topic-specific text
lives in the content pack as JSON, in both locales. See
...
Deployed on Vercel. Set `KAFKEX_PASSPHRASE` and `DATABASE_URL` in the
